...
def twitter_query(output, input_file, auth_file):
    ...
    #query the tweets
    query_user_follower_ids(output, id_list, auth_file)

def query_user_follower_ids(output, id_list, auth_file):
    ...
    for userid in id_list:
        ...
        if not userid == '':
            try:
                count = 0
                for item in Cursor(api_pool.followers_ids, id=userid).items():
...
def parse_args(args):
    ...
    parser.add_argument('-l', '--log', dest='log', default=os.path.expanduser('~/pylogs/query_user_follower_ids_'+currentdate+'.log'), help='This is the path to where your output log should be. Required')
